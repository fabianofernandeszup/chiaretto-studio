...
def verificar_e_modificar_yaml(nome_diretorio):
    ...
    for nome_arquivo in arquivos_encontrados:
        ...
        ajustar_inputs_select(caminho_arquivo, conteudo)

        ajustar_docs(nome_diretorio, caminho_arquivo, conteudo)

        adjust_connection_interface(caminho_arquivo, conteudo)

def ajustar_docs(nome_diretorio, caminho_arquivo, conteudo):
    # Ajustar arquivos
...
def ajustar_inputs_select(caminho_arquivo, conteudo):
    print('########## Ajustando inputs select')
    if 'spec' in conteudo and 'inputs' in conteudo['spec']:
        for input_item in conteudo['spec']['inputs']:
            if input_item.get('type') == 'text' and 'items' in input_item:
                input_item['type'] = 'select'

    editar_arquivo_yaml(caminho_arquivo, conteudo)


def adjust_connection_interface(caminho_arquivo, conteudo):
    if 'spec' in conteudo and 'requires' in conteudo['spec'] and 'connection-interface' in conteudo['spec']['requires']:
        connections = conteudo['spec']['requires'].pop('connection-interface')
        conteudo['spec']['requires']['connections'] = [{'type': conn, 'alias': f'alias-default-{conn}'} for conn in
                                                   connections]
